...
input_ext = os.path.splitext(sys.argv[1])[
    1
].lower()  # Get file ending of original image
output_ext = os.path.splitext(sys.argv[2])[
    1
].lower()  # Get file ending of where the new image will be

if input_ext not in [".jpg", ".jpeg", ".png"] or output_ext not in [
    ".jpg",
    ".jpeg",
    ".png",
]:  # Check that file endings are either jpg or png
...
photo.paste(
    shirt, shirt
)  # Overlays the shirt image. Asks twice, first time for image and second time for what pixels to change
