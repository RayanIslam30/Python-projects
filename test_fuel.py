...
def test_convert():
    ...
    with pytest.raises(ValueError): #Check that x cannot be negative, or exception is raised
        convert("-1/2")

    with pytest.raises(ValueError): #Check that x/y cannot be greater than 1, or exception is raised
        convert("2/1")
